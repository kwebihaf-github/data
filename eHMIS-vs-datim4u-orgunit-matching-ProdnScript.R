...
# export files for manual matching
DATIM4UMatched_2 <-as.data.frame(eHMIS_DATIM4U_orgunit_matching_all_temp_5$DATIM4U_orgunit_uid)
colnames( DATIM4UMatched_2) <-"ServiceOutlet"

DATIM4UforManualMatching <- DATIM4UorgunitDataSubset %>% 
  dplyr::anti_join(DATIM4UMatched_2)

# export to csv
data.table::fwrite(DATIM4UforManualMatching,
                   file="~/Downloads/DATIM4U/DATIM/DATIM4UforManualMatching.csv",
                   row.names = FALSE)

data.table::fwrite(eHMISorgunitDataSubset,
                   file="~/Downloads/DATIM4U/DATIM/eHMISorgunitDataSubset.csv",
                   row.names = FALSE)

data.table::fwrite(DATIM4UorgunitDataSubset,
                   file="~/Downloads/DATIM4U/DATIM/DATIM4UorgunitDataSubset.csv",
                   row.names = FALSE)

# read the manual matched file

temp = tempfile(fileext = ".xlsx")
dataURL <- "https://github.com/kwebihaf-github/data/raw/master/DATIM4UforManualMatching.xlsx"
download.file(dataURL, destfile=temp, mode='wb')

DATIM4UforManualMatching_2 <- readxl::read_excel(temp, sheet =1)

# subset for where eHMIS_UID column is not blank
DATIM4UforManualMatching_2  <- DATIM4UforManualMatching_2 %>% 
  dplyr::filter(!is.na(eHMIS_UID))

# select wanted columns from the subset
DATIM4UforManualMatching_2 <- DATIM4UforManualMatching_2 %>% 
                              dplyr::select(eHMIS_UID,ServiceOutlet)

# combine the previous matches with the manual matching

eHMIS_DATIM4U_orgunit_matching_all_temp_6 <-data.table::rbindlist(list(eHMIS_DATIM4U_orgunit_matching_all_temp_5,DATIM4UforManualMatching_2))

# export the combined to csv
data.table::fwrite(eHMIS_DATIM4U_orgunit_matching_all_temp_6,
                   file="~/Downloads/DATIM4U/DATIM/eHMIS_DATIM4U_orgunit_matching_all_temp_file-04.csv",
                   row.names = FALSE)
